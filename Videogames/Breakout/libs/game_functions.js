...
function colitionType(obj1,obj2){
    const distanceX = Math.abs(obj1.position.x - obj2.position.x);
    const distanceY = Math.abs(obj1.position.y - obj2.position.y);

    const sumHalfWidths = obj1.halfSize.x + obj2.halfSize.x;
    const sumHalfHeights = obj1.halfSize.y + obj2.halfSize.y;

    const colitionX = sumHalfWidths - distanceX;
    const colitionY = sumHalfHeights - distanceY;

    if(colitionX<colitionY){
        return 'side';
    }
    else if(colitionX>colitionY){
        return 'top';
    }
    else{
        return 'corner';
